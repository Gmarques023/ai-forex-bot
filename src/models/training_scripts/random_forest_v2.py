...
# Criar uma coluna para o Target da próxima vela
df['Target'] = df['close'].shift(-1) > df['open'].shift(-1)
df['Target'] = df['Target'].astype(int)

# Remover a última linha, pois ela não tem um Target da próxima vela
df = df[:-1]
...
def create_rolling_window_features(data, window_size=60):
    ...
    for i in range(window_size, len(data)):
        ...
        # Target da vela atual
        rolling_targets.append(data['Target'].iloc[i])
...
joblib.dump(model, '../trained_models/random_forestv4.pkl')
